                ...
                {{-- @foreach(array_chunk($articles, 3) as $chunk) --}}
                    ...
                    {{-- @php echo '<div class="row">'; @endphp --}}
                        ...
                        @foreach ($articles as $article)
                            @php $textarticle = Str::limit($article->textArticle, 100, '....') @endphp
                            @php echo '<div class="col-lg-4 col-md-4 tm-info-box">'; @endphp
                                <img id='preview-image-before-upload' src='{{ url('public/images/'.$article->imgFilepath) }}' alt='preview image' class='img-center'>
                                @php echo "<h5 class='article-title mb-4'>$article->titleArticle</h5>"; @endphp
                                {{-- {!! $article->textArticle !!}} --}}
                                @php echo "<span class='article-p'><p class = 'p mb-4'>$article->textArticle</p></span>"; @endphp
                                @php echo "<p class='article-date mb-4'>$article->created_at</p>"; @endphp
                                <a href='{{ url('/article/detail/'.$article->idArticle) }}' class ='tm-info-box-a'>READ MORE</a>
                            @php echo '</div>'; @endphp
                        @endforeach
                   {{-- @php echo '</div>'; @endphp --}}
                    </div>
